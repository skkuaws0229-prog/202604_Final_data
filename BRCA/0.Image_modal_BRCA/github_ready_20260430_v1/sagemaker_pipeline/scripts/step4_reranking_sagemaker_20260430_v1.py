...
def feature_frame(df: pd.DataFrame, emb_cols: list[str], include_image: bool = True) -> pd.DataFrame:
    cols = [c for c in ["ensemble_pred", "component_pred_std"] if c in df.columns]
    if include_image:
        cols += emb_cols
    if not cols:
...
def rerank_top30(top30: pd.DataFrame, patient_embeddings: pd.DataFrame, model, feature_cols: list[str], emb_cols: list[str]) -> pd.DataFrame:
    ...
    for _, drug in top30.iterrows():
        for _, emb in patient_embeddings.iterrows():
            ...
            row["ensemble_pred"] = float(drug.get("drug_level_score", drug.get("ensemble_pred", 0.0)))
            row["component_pred_std"] = float(drug.get("prediction_std_mean", 0.0))
            rows.append(row)
...
def main() -> int:
    ...
    ensure_processing_output_dirs()
    logger = setup_logging(Path("/opt/ml/processing/output/logs"), "step4_reranking_sagemaker")
